...
from models.pcr_net import build_model
import utils
...
def train(model, data_loader, optimizer, epoch, device, config):
    ...
    loss_weights = config.get('loss_weights', {'global': 1.0, 'local': 1.0, 'ara': 0.1, 'mim': 1.0})
    ...
    for i,(image, caption, img_pid) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_glb, loss_loc, loss_ara, loss_mim = model(image, caption, idx=img_pid)
        loss_glb = loss_glb * loss_weights['global']
        loss_loc = loss_loc * loss_weights['local']
        loss_ara = loss_ara * loss_weights['ara']
        loss_mim = loss_mim * loss_weights['mim']
        loss = loss_glb + loss_loc + loss_ara + loss_mim
...
def main(args, config):
    ...
    model = build_model(pretrained=config['pretrained'],mode=mode,num_classes=num_classes, image_size=config['image_size'], vit=config['vit'],
                             ...
                             topk_image_ratio=config.get('topk_image_ratio', 0.5),
                             topk_text_ratio=config.get('topk_text_ratio', 0.5),
                             max_patch=config.get('max_patch', 256),
                             max_words=config.get('max_words', 72))
